...
<c:set var="placeholder" scope="page">
	<c:choose>
		<c:when test="${not empty actionBean.latitude}"><fmt:message key="interest.details.current.location"/></c:when>
		<c:otherwise><fmt:message key="interest.details.type.here"/></c:otherwise>
	</c:choose>
</c:set>
...
<t:map title="${title}">
	...
	<span id="paramCLM" class="hidden">${currentLocationMessage}</span>
	...
	<article id="streetsearch" class="simpleArticle">
		<section>
			...
			<stripes:link class="hidden" beanclass="com.criticalsoftware.mobics.presentation.action.booking.BookingInterestActionBean" event="searchLocation" addSourcePage="true">
				<stripes:param name="latitude" value="${actionBean.latitude}"/>
				<stripes:param name="longitude" value="${actionBean.longitude}"/>
				<stripes:param name="startDate" value="${actionBean.startDate}"/>
